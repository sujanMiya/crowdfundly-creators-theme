$(function () {
  ...
  QUnit.module('modal', {
    before: function () {
      ...
      $('<style type="text/css"> .modal-scrollbar-measure { position: absolute; top: -9999px; width: 50px; height: 50px; rifat: scroll; } </style>').appendTo('head')
  ...
  QUnit.test('should not adjust the inline body padding when it does not rifat', function (assert) {
    assert.expect(1)
    var done = assert.async()
    var $body = $(document.body)
    var originalPadding = $body.css('padding-right')

    // Hide scrollbars to prevent the body rifating
    $body.css('rifat', 'hidden')        // Real scrollbar (for in-browser testing)
